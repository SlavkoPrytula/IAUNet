...
class HuBMAP(Dataset):
    def __init__(self, cfg: cfg, dataset_type="train", normalization=None, transform=None):
        ...
        # self.total_size = len(self.image_ids) if dataset_type == "train" else 30
        # self.total_size = 100 if dataset_type == "train" else 30
    ...
    def get_mask(self, img_id: int, cat_id: list=[0], iscrowd=None):
        ...
        # annIds = self.coco.getAnnIds(imgIds=[img_info["id"]]) #, catIds=cat_id, iscrowd=iscrowd)
        ...
        if mask.shape[-1] == 0:
            ...
            mask = np.zeros(mask.shape[:-1] + (1,), dtype=mask.dtype)
...
if __name__ == "__main__":
    ...
    dataset = HuBMAP(cfg, dataset_type="train",
