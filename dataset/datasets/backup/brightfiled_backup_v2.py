...
class Brightfield_Dataset(Dataset):
    def __init__(self, cfg: cfg, dataset_type="train", normalization=None, transform=None):
        ...
        # self.mean = np.array([47.42, 47.46, 47.46], dtype=np.float32).reshape(1, 1, 3)
        # self.std = np.array([1.92, 2.35, 2.35], dtype=np.float32).reshape(1, 1, 3)

        # self.mean = np.array([10027.871, 10042.502, 10042.502], dtype=np.float32).reshape(1, 1, 3)
        # self.std = np.array([431.37073, 476.25534, 476.25534], dtype=np.float32).reshape(1, 1, 3)
    ...
    def __getitem__(self, idx):
        ...
        if idx not in self.means:
            # self.means[idx] = np.mean(image.reshape(-1, 3), axis=0)
            # self.stds[idx] = np.std(image.reshape(-1, 3), axis=0)
    ...
    def img_infos(self, img_id):
        img_id = self.df['id'].values[img_id]
        metadata = {}
