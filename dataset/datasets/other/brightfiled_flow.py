...
print('Loading flow maps...')
flow_maps = np.load(join(cfg.flow_masks, f'{cfg.flow_masks_name}.npy'), allow_pickle=True)
print('Finished loading flow maps...')

print('Preparing flow maps...')
dx_grad = []
dy_grad = []
for i in tqdm(range(28)):
    dx_flow_map = flow_maps[i][:, :, 0, 0]
    dy_flow_map = flow_maps[i][:, :, 1, 0]
    for j in range(1, flow_maps[i].shape[-1]):
        dx_flow_map[flow_maps[i][:, :, 0, j] != 0] = flow_maps[i][:, :, 0, j][flow_maps[i][:, :, 0, j] != 0]
        dy_flow_map[flow_maps[i][:, :, 1, j] != 0] = flow_maps[i][:, :, 1, j][flow_maps[i][:, :, 1, j] != 0]

    dx_flow_map = np.expand_dims(dx_flow_map, -1)
    dy_flow_map = np.expand_dims(dy_flow_map, -1)

    dx_grad.append(dx_flow_map)
    dy_grad.append(dy_flow_map)

dx_grad_all = np.stack(dx_grad, 0)
dy_grad_all = np.stack(dy_grad, 0)
print('Finished preparing flow maps...')
...
class Brightfield_Dataset(Dataset):
    def __init__(self, df, run_type, img_size, normalization, transform):
        ...
        self.dx_grad, self.dy_grad = dx_grad_all.copy()[df['mask_id'].values], dy_grad_all.copy()[df['mask_id'].values]
    ...
    def __getitem__(self, idx):
        ...
        dx_grad = self.dx_grad[idx]
        dy_grad = self.dy_grad[idx]
        

        if self.transform:
            data = self.transform(image=bf_image, image1=pc_image, mask=cyto_mask, mask1=nuc_mask, dx_grad=dx_grad, dy_grad=dy_grad)
            bf_image = data['image']
            pc_image = data['image1']
            cyto_mask = data['mask']
            nuc_mask = data['mask1']
            
            dx_grad = data['dx_grad']
            dy_grad = data['dy_grad']
            
        cyto_mask = self.filter_empty_masks(cyto_mask)
        nuc_mask = self.filter_empty_masks(nuc_mask)
        
        cond_mask = flatten_mask(cyto_mask, -1)
        cond_mask[cond_mask > 1] = 1
        
#         _cond_mask = flatten_mask(nuc_mask, -1)
#         _cond_mask[_cond_mask > 1] = 1
#         cond_mask += _cond_mask
        ...
        cond_mask = np.transpose(cond_mask, (2, 0, 1))
        cond_mask = torch.tensor(cond_mask, dtype=torch.float32)
        
        dx_grad = np.transpose(dx_grad, (2, 0, 1))
        dx_grad = torch.tensor(dx_grad, dtype=torch.float32)
        dy_grad = np.transpose(dy_grad, (2, 0, 1))
        dy_grad = torch.tensor(dy_grad, dtype=torch.float32)

        return bf_image, pc_image, cyto_mask, nuc_mask, cond_mask, dx_grad, dy_grad
