...
# @DECODERS.register(name='iadecoder_ml_fpn/experimental/dual_path')
...
#             mask_feats = mask_feats.contiguous()
...
#                     self.forward_prediction_heads(query_feat, mask_feats)
...
#         outputs_class, outputs_mask, outputs_boxes = \
#             self.forward_prediction_heads(query_feat, mask_features.flatten(2).permute(2, 0, 1))
...
#         HW = mask_features.shape[1]
#         H, W = int(HW ** 0.5), int(HW ** 0.5)
#         outputs_mask = torch.einsum("bqc,blc->bql", mask_embed, mask_features)
#         outputs_mask = outputs_mask.reshape(-1, N, H, W)
...
class IADecoder(BaseDecoder):
    ...
    def forward_last_layer(self, pixel_feats, query_feat, query_embed, pos, layer_idx):      
        # pixel ca.
        pixel_feats, pixel_ca_attn = self.transformer_pixel_cross_attention_layers[layer_idx - self.n_levels + 1](
            pixel_feats, query_feat, 
            memory_mask=None, 
            memory_key_padding_mask=None, 
            pos=query_embed, query_pos=pos
