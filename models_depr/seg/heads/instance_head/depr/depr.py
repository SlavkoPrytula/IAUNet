...
#     def forward_prediction_heads(self, query_features, pixel_features):
#         pred_logits = self.cls_score(query_features)
#         pred_inst_query = self.inst_kernel(query_features)
#         pred_scores = self.objectness(query_features)
#         pred_bboxes = self.bbox_pred(query_features)

#         pred_masks = torch.einsum("bqc,bchw->bqhw", pred_inst_query, pixel_features)

#         # NOTE: prediction is of higher-resolution
#         # [B, Q, H, W] -> [B, Q, H*W] -> [B, h, Q, H*W] -> [B*h, Q, HW]
#         # must use bool type
#         # If a BoolTensor is provided, positions with ``True`` are not allowed to attend while ``False`` values will be unchanged.
#         attn_mask = (pred_masks.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1) < 0.5).bool()
#         attn_mask = attn_mask.detach()

#         return pred_logits, pred_inst_query, pred_scores, pred_bboxes, pred_masks, attn_mask
...
#         if attn_mask:
#             attn_mask = F.interpolate(attn_mask, size=(H, W), 
...
#         pred_logits, pred_inst_query, pred_scores, pred_bboxes, pred_masks, attn_mask = self.forward_prediction_heads(inst_features, pixel_features)
...
#             'objectness_scores': pred_scores,
#             'kernels': {
#                 'instance_kernel': pred_inst_query,
#                 },
#             'bboxes': {
#                 'instance_bboxes': pred_bboxes
#                 },
