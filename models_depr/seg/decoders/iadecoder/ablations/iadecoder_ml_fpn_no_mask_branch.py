...
@DECODERS.register(name='iadecoder_ml_fpn_no_mask_branch')
class IADecoder(IADecoder):
    ...
    def _forward(self, skips):
        for i in range(self.n_levels):
            ...
            if i != 0:
                results = self.instance_head[i](inst_feats, x, inst_embed)
                inst_embed = results["inst_feats"]['instance_feats']
            else:
                results = self.instance_head[i](inst_feats, x)
                inst_embed = results["inst_feats"]['instance_feats']

            x = results['mask_pixel_feats']
