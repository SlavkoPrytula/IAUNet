...
@DECODERS.register(name='iadecoder_ml_fpn/experimental/dual_path_deep_supervision')
class IADecoder(BaseDecoder):
    ...
    def forward_one_layer(self, pixel_feats, query_feat, mask_features, query_embed, pos, pos_mask_feats, layer_idx):      
        # pixel ca.
        mask_features, pixel_ca_attn = self.transformer_pixel_cross_attention_layers[layer_idx](
            mask_features, query_feat, 
            memory_mask=None, 
            memory_key_padding_mask=None, 
            pos=query_embed, query_pos=pos_mask_feats
            )
        
        # pixel ffn.
        mask_features = self.transformer_pixel_ffn_layers[layer_idx](mask_features)
        ...
        attn = {
            'pixel_ca_attn': pixel_ca_attn,
            'query_ca_attn': query_ca_attn,
            'query_sa_attn': query_sa_attn
        }
    
        return pixel_feats, query_feat, mask_features, attn
    ...
    def _forward(self, features):
        ...
        # transformer decoder.
        # ------------------------
        # this is a transformer decoder scaled in snake style.
        # [x], [q] -> [dec_layer_0.0] -> [dec_layer_1.0] -> ... -> [dec_layer_n.0] -> 
        #          -> [dec_layer_0.1] -> [dec_layer_1.1] -> ... -> [dec_layer_n.1] -> ...
        # ------------------------
        ...
        #             self.forward_prediction_heads(query_feat, mask_features)
        ...
        B, _C, _H, _W = mask_features.shape
        pos_mask_feats = self.pe_layer(mask_features, None)
        pos_mask_feats = mask_features.flatten(2).permute(2, 0, 1)

        # ------------------------
        # this is a transformer decoder scaled in width.
        # [x], [q] -> [dec_layer_0.0] -> [dec_layer_0.1] -> ... -> [dec_layer_0.n]
        # ------------------------
        for i, x in enumerate(src):
            for _, j in enumerate(range(self.dec_layers)):
                ...
                mask_features = mask_features.flatten(2).permute(2, 0, 1)
                
                x, query_feat, mask_features, attn = self.forward_one_layer(x, query_feat, mask_features, 
                                                                      query_embed, pos, pos_mask_feats, layer_idx)
                
                x = x.permute(1, 2, 0).view(B, C, H, W)

                mask_features = mask_features.permute(1, 2, 0).view(B, _C, _H, _W)
