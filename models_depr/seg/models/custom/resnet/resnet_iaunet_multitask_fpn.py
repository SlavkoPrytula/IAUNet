...
@MODELS.register(name="resnet_iaunet_multitask_fpn")
class IAUNet(nn.Module):
    def __init__(
        ...
        self.bridge = nn.Sequential(
            ...
            # DoubleConv(embed_dims[4], embed_dims[4]),
        ...
        for i in range(self.n_levels):
            skip_layer = nn.Sequential(
                nn.Conv2d(embed_dims[i], embed_dims[i], kernel_size=1)      
            )
            self.skip_blocks.append(skip_layer)
    ...
    def forward(self, x, idx=None):
        ...
        for i in range(self.n_levels):
            ...
            skip_x = self.skip_blocks[i](skips[-(i + 1)])
            x = x + skip_x
