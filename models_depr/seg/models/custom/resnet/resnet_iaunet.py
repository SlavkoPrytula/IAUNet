...
@MODELS.register(name="resnet_iaunet")
class IAUNet(nn.Module):
    def __init__(
        ...
        # self.middleConv = DoubleConvModule(self.embed_dims[-1], self.embed_dims[-1])
        ...
        for i in range(self.n_levels):
            ...
            upconv = DoubleConv_v2(in_channels, out_channels)
            self.up_conv_layers.append(upconv)

            up_se = SE_block(num_features=out_channels)
            self.up_se_blocks.append(up_se)


        embed_dims = [1024, 512, 256, 64, 64]
        # mask branch.
    ...
    def forward(self, x, idx=None):
        ...
        masks = torch.bmm(
            kernel,    # (B, N, 128)
            mb.view(B, C, H * W)   # (B, 128, [HW])
        ) # -> (B, N, [HW])
        masks = masks.view(B, N, H, W)  # (B, N, [HW]) -> (B, N, H, W)

        masks = nn.UpsamplingBilinear2d(scale_factor=2)(masks)
