...
@MODELS.register(name="custom-truncated_decoder-iaunet_v2")
class IAUNet(BaseModel):
    def __init__(self, cfg: cfg):
        ...
        for i in range(self.n_levels):
            if i == 0:
                ...
                mask_branch = MaskBranch(
                    in_channels=embed_dims[i] + self.mask_dim + 2, 
                    out_channels=self.mask_dim, 
                    num_convs=self.num_convs
                    )
    ...
    def forward(self, x):
        ...
        mask_feats = torch.cat([coord_features, x], dim=1)
        mask_feats = self.mask_branch[0](mask_feats)
        ...
        for i in range(self.n_levels-1):
            ...
            mask_feats = self.mask_branch[i+1](mask_feats)
            ...
            inst_feats = self.instance_branch[i+1](inst_feats)
