...
@MODELS.register(name="custom-truncated_decoder-iaunet_double_decoder")
class IAUNet(BaseModel):
    def __init__(self, cfg: cfg):
        ...
        for i in range(self.n_levels-1):
            ...
            self.mask_branch.append(upconv)
        ...
        for i in range(self.n_levels-1):
            ...
            self.instance_branch.append(upconv)
    ...
    def _init_weights(self):
        for modules in [self.mask_branch, self.instance_branch, self.bridge]:
    ...
    def forward(self, x):
        ...
        for i in range(self.n_levels-1):
            ...
            if i != 0:
                ...
                mask_feats = torch.cat([mask_feats, skips[-(i + 1)]], dim=1)
                
                coord_features = self.compute_coordinates(mask_feats)
                mask_feats = torch.cat([coord_features, mask_feats], dim=1)
                mask_feats = self.mask_branch[i](mask_feats)
            else:
                mask_feats = torch.cat([x, skips[-(i + 1)]], dim=1)
            ...
            if i != 0:
                ...
                inst_feats = torch.cat([inst_feats, skips[-(i + 1)]], dim=1)
                ...
                inst_feats = torch.cat([x, skips[-(i + 1)]], dim=1)
                coord_features = self.compute_coordinates(inst_feats)
                inst_feats = torch.cat([coord_features, inst_feats], dim=1)
                inst_feats = self.instance_branch[i](inst_feats)
