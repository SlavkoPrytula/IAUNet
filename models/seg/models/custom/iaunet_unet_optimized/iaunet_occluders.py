...
from models.seg.heads.instance_head.instance_occluder_head import InstanceBranch, PriorInstanceBranch
    ...
    def forward(self, x, idx=None):
        ...
        def go_up(x):
            for i in range(self.n_levels):
                ...
                if i == self.n_levels - 1:
                    logits, mask_kernel, occl_kernel, scores, iam = self.instance_branch(inst_feats, idx)
                    mb = self.projection(mb)

            return x, mb, (logits, mask_kernel, occl_kernel, scores, iam)
    
        # cyto
        x, mask_features, (logits, mask_kernel, occl_kernel, scores, iam) = go_up(x)
