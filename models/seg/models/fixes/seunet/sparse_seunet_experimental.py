...
from models.seg.heads.instance_head import InstanceBranchNoIAM, PriorInstanceBranch, IAM
...
@MODELS.register(name="sparse_seunet_experimental")
class SparseSEUnet(BaseModel):
    """
    Base SparseUnet + low level instance activation maps
    """
    def __init__(
        self,
        cfg: cfg,
    ...
    def forward(self, x, idx=None):
        ...
        def go_up(x):
            ...
            iam = F.interpolate(iam, size=mb.shape[-2:], mode="bilinear", align_corners=False)
            logits, kernel, scores, iam = self.instance_branch(inst_feats, iam)
