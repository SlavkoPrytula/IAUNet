...
@MODELS.register(name="sparse_seunet_residual")
class SparseSEUnet(BaseModel):
    def __init__(
        ...
        for i in range(self.n_levels):
            if i == 0:
                self.instance_branch.append(InstanceBranch(dim=mask_dim, kernel_dim=self.kernel_dim, num_masks=self.num_masks))
            else:
                self.instance_branch.append(InstanceBranch(dim=mask_dim + self.num_masks, kernel_dim=self.kernel_dim, num_masks=self.num_masks))
        ...
        def go_up(x):
            for i in range(self.n_levels):
                ...
                if i != 0:
                    ...
                    iam_feats = nn.UpsamplingBilinear2d(scale_factor=2)(iam_feats)
                    _inst_feats = torch.cat([inst_feats, iam_feats], dim=1)
                    _, _, _, iam_feats = self.instance_branch[i](_inst_feats)
                else:
                    mask_feats = self.mask_branch[i](x)
                    inst_feats = self.prior_instance_branch[i](x)
                    
                    _, _, _, iam_feats = self.instance_branch[i](inst_feats)
