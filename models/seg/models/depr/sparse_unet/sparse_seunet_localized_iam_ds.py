...
@MODELS.register(name="sparse_unet-deep_supervision-sparse_seunet_localized_iam_ds")
class SparseSEUnet(BaseModel):
    ...
    def forward(self, x, idx=None):
        ...
        def go_up(x):
            ...
            for i in range(self.n_levels):
                ...
                if i in [0, 1, 2, 3]:
                    ...
                    if i == 0:
                        # global features
                        inst_feats_all.append(inst_feats)
                    ...
                    inst_logits, inst_kernel, scores, inst_iam, inst_coord = self.instance_branch[i](inst_feats, loc_iam)
                    ...
                    kernel.append(inst_kernel)
                    logits.append(loc_cate)

                    # mb = self.mask_branch[i](x)
                    # mb = self.projection(mb)

                    # mask_feats.append(mb)


                if i in [4]:
                    # local features
                    inst_feats = self.prior_instance_branch[i](x)
                    # inst_feats = nn.MaxPool2d(16)(inst_feats)

                    # global + local features
                    # inst_feats = inst_feats + inst_feats_all[0]
                    # inst_feats = torch.cat([inst_feats, inst_feats_all[0]], dim=1)
            ...
            return x, mask_feats, (logits, kernel, scores, iam, coords)
        ...
        # for i, (k, m) in enumerate(zip(kernel, mask_features)):
        #     B, C, H, W = m.shape
