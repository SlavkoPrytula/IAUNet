...
@DECODERS.register(name='iadecoder_ml_fpn_no_inst_branch')
class IADecoder(IADecoder):
    ...
    def _forward(self, skips):
        for i in range(self.n_levels):
            ...
            if i != 0:
                results = self.instance_head[i](x, mask_feats, inst_embed)
                inst_embed = results["inst_feats"]['instance_feats']
            else:
                results = self.instance_head[i](x, mask_feats)
                inst_embed = results["inst_feats"]['instance_feats']

            mask_feats = results['mask_pixel_feats']
            x = results['inst_pixel_feats']
