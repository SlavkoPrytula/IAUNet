...
nextvit_settings = {
    'S': [[64, 32, 64], [3, 4, 10, 3], 0.2],  # [embed_dims, stem_chs, depths, path_dropout]
    'B': [[64, 32, 64], [3, 4, 20, 3], 0.2],
    'L': [[64, 32, 64], [3, 4, 30, 3], 0.2],
}
...
class NextViT(nn.Module):
    ...
    def __init__(self, model_name: str = 'B', attn_drop=0, drop=0, num_classes=1000,
                 strides=[1, 2, 2, 2], sr_ratios=[8, 4, 2, 1], head_dim=32, mix_block_ratio=0.75, resume='',
                 with_extra_norm=True, frozen_stages=-1, norm_eval=False, norm_cfg=None,
                 ):
        super(NextViT, self).__init__()

        stem_chs, depths, path_dropout = nextvit_settings[model_name]

        # TODO: change embed dims based on architecture
        self.embed_dims = [96, 256, 512, 1024]
