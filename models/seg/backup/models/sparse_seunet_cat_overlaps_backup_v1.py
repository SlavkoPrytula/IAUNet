...
class SparseSEUnet(nn.Module):
    def __init__(
        ...
        for i in range(self.n_levels):
            self.fusing.append(nn.Linear(208+208, 208))
        
        # mask branch.
        self.mask_branch = nn.ModuleList([])
        for i in range(self.n_levels):
            if i == 0:
                self.mask_branch.append(MaskBranch(in_channels=208))
            else:
                self.mask_branch.append(MaskBranch(in_channels=208+128))
        
        # instance features.
        self.prior_instance_branch = nn.ModuleList([])
        for i in range(self.n_levels):
            if i == 0:
                self.prior_instance_branch.append(PriorInstanceBranch(in_channels=208 + 208))
            else:
                self.prior_instance_branch.append(PriorInstanceBranch(in_channels=464 + 208))
    ...
    def forward(self, x, idx=None):
        ...
        def go_up(x, overlap_feats):
            for i in range(self.n_levels):
                if self.coord_conv:
                    ...
                    coord_features = self.compute_coordinates(overlap_feats)
                ...
                # add conditional overlap information 
                _x = torch.cat([x, overlap_feats], dim=1)
                # x = F.relu_(self.fusing[i](_x))
        ...
        # cyto
        x, overlap_feats, mask_features, (logits, kernel, scores, iam) = go_up(x, x)
