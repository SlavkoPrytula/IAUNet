...
#             skip_conv = DoubleConv(dim, self.n_filters)
#             self.skip_conv_layers.append(skip_conv)

#             skip_se = SE_block(num_features=self.n_filters)            
#             self.skip_se_blocks.append(skip_se)
...
#             if i == 0:
#                 self.mask_branch.append(MaskBranch(in_channels=dim, out_channels=self.mask_dim))
#             else:
#                 self.mask_branch.append(MaskBranch(in_channels=dim + self.mask_dim, out_channels=self.mask_dim))
        
#         # instance features.
#         self.prior_instance_branch = nn.ModuleList([])
#         for i in range(self.n_levels):
#             if i == 0:
#                 self.prior_instance_branch.append(PriorInstanceBranch(in_channels=dim, out_channels=self.mask_dim))
#             else:
#                 self.prior_instance_branch.append(PriorInstanceBranch(in_channels=dim + self.mask_dim, out_channels=self.mask_dim))

#         # instance branch.
#         self.instance_branch = InstanceBranch(dim=self.mask_dim, kernel_dim=self.kernel_dim, num_masks=self.num_masks)

#         # out mask projection.
#         self.projection = nn.Conv2d(self.mask_dim, self.mask_dim, kernel_size=1)
...
#                 x = self.skip_conv_layers[i](x)
#                 x = self.skip_se_blocks[i](x)
