...
#         )
        ...
        self.instance_branch = nn.ModuleList([
            InstanceBranch(in_channels=208, kernel_dim=128, num_masks=num_masks),
            InstanceBranch(in_channels=308, kernel_dim=128, num_masks=num_masks),
            InstanceBranch(in_channels=308, kernel_dim=128, num_masks=num_masks),
            InstanceBranch(in_channels=308, kernel_dim=128, num_masks=num_masks),
            InstanceBranch(in_channels=308, kernel_dim=128, num_masks=num_masks),
        ])
        
        self.kernel_conv = nn.ModuleList([
        ...
        #     nn.Conv2d(308, 208, kernel_size=1, stride=1),
        #     nn.Conv2d(308, 208, kernel_size=1, stride=1),
        #     nn.Conv2d(308, 208, kernel_size=1, stride=1),
        #     nn.Conv2d(308, 208, kernel_size=1, stride=1),
        # ])
    ...
    def forward(self, x):
        ...
        def go_up(x):
            ...
            for i in range(self.n_levels):
                ...
                #     print(x.shape, iam.shape)
                #     # concat 
                #     x = torch.cat([x, iam], dim=1)

                #     logits, kernel_upper, scores, iam = self.instance_branch[i](x)
                    
                #     kernel = torch.cat([kernel, kernel_upper], dim=-1)
                #     kernel = self.kernel_conv[i-1](kernel.transpose(1, 2)).transpose(1, 2)

                #     x = self.instance_conv[i-1](x)
                #     print(x.shape)
                # else:
                #     _, kernel, _, iam = self.instance_branch[i](x)
                #     # iam_maps shape: (B, N, Hx, Wx)
                ...
                if i != 0:
                    ...
                    # print(x.shape, iam.shape)
                    # concat 
                    feat_iam = torch.cat([x, iam], dim=1)

                    logits, kernel, scores, iam = self.instance_branch[i](feat_iam)

                    # x = self.instance_conv[i-1](x)
                    # print(x.shape)
                else:
                    _, _, _, iam = self.instance_branch[i](x)
                    # iam_maps shape: (B, N, Hx, Wx)
        ...
        return flow_pred, logits, masks, scores, iam
