...
def mock_data():
    outputs = {
        ...
        "pred_masks": torch.stack([
            # create_rectangular_mask((128, 128), (3, 3, 6, 6)),
            # create_rectangular_mask((128, 128), (5, 5, 8, 8)),
            # create_rectangular_mask((128, 128), (0, 0, 3, 3)),
            # create_rectangular_mask((128, 128), (7, 7, 9, 9)),
            # create_rectangular_mask((128, 128), (1, 1, 4, 4))

            create_rectangular_mask((128, 128), (0, 0, 0, 0)),
            create_rectangular_mask((128, 128), (0, 0, 0, 0)),
            create_rectangular_mask((128, 128), (0, 0, 0, 0)),
            create_rectangular_mask((128, 128), (0, 0, 0, 0)),
            create_rectangular_mask((128, 128), (0, 0, 0, 0))
    ...
    indices = [(torch.tensor([4, 0, 1]), torch.tensor([0, 1, 2]))]

    return outputs, targets, indices


def test():
    cfg.model.criterion.losses = ["labels", "masks"]
    criterion = CRITERIONS.build(cfg.model.criterion)
    outputs, targets, indices = mock_data()

    indices = criterion.matcher(outputs, targets, (128, 128))
    print(indices)

    num_masks = sum(len(t['masks']) for t in targets)  # Calculate total number of masks across all targets
    # loss_results = criterion.loss_labels(outputs, targets, indices, num_masks, (128, 128))
    loss_results = criterion(outputs, targets, (128, 128))
    print(loss_results)
