...
def run(cfg: cfg):
    ...
    evaluator = EVALUATORS.get(cfg.model.evaluator.type)(cfg=cfg.model.evaluator)  # from config
    ...
    for i in range(1, 7):
        img_path = valid_dataset.img_infos(i-1)["img_path"]
        fname, name = osp.splitext(osp.basename(img_path))
        out_file = join(cfg.visuals_dir, f'{fname}.jpg')

        H = valid_dataset.img_infos(i-1)["height"]
        W = valid_dataset.img_infos(i-1)["width"]

        img = valid_dataset[i-1]["image"][0]
        # img = cv2.imread(img_path, -1).astype(np.float32)
        # if len(img.shape) != 3:
        #     img = np.expand_dims(img, -1)
        # img = valid_dataset.get_image(i-1)
        # img = np.transpose(img, (2, 0, 1))

        # img = np.zeros((512, 512))
        total_width = W * 2  # Assuming you have two images to stitch
        dpi = 100
        fig_width_inches = total_width / dpi
        fig_height_inches = H / dpi

        # fig, ax = plt.subplots(1, 2, figsize=[fig_width_inches, fig_height_inches], dpi=dpi)
        ...
        fig.savefig(out_file, bbox_inches='tight', pad_inches=0)  # Save without padding
        plt.close(fig)
        ...
        #     a.axis('tight')
        #     a.axis('off')
        #     a.set_position([0, 0, 1, 1])  # Make the Axes instance hugging the figure borders

        # fig.canvas.draw()  # Update the figure
        # fig.savefig(out_file, dpi='figure', bbox_inches='tight', pad_inches=0)
        # for ax in fig.axes:
        #     ax.axis('off')
        #     ax.axis('tight')
        #     ax.margins(0,0)
