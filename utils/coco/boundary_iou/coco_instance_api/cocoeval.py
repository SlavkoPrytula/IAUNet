...
# All rights reserved.
# Modified by Bowen Cheng to compute Boundary AP.
...
class COCOeval:
    ...
    #  iouType    - ['segm'] set iouType to 'segm', 'bbox', 'keypoints' or 'boundary'
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm', dilation_ratio=0.02):
        '''
        ...
        # Augment boundary.
        self.use_boundary_iou = iouType == "boundary"
        if self.use_boundary_iou:
            if not self.cocoGt.get_boundary:
                self.cocoGt.get_boundary = self.use_boundary_iou
                self.cocoGt.dilation_ratio = dilation_ratio
                self.cocoGt.createIndex()
            if not self.cocoDt.get_boundary:
                self.cocoDt.get_boundary = self.use_boundary_iou
                self.cocoDt.dilation_ratio = dilation_ratio
    ...
    def _prepare(self):
        ...
        # convert ground truth to mask if iouType == 'segm' or p.iouType == 'boundary'
