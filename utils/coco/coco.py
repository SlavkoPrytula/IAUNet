...
class COCO:
    def __init__(self, coco_data=None, annotation_file=None, verbose=True):
        """
    ...
    # def getMasks(self, anns, alpha=1):
    #     """
    ...
    #                     masks.append(np.dstack( (img, m*alpha) ))
    ...
    # def getMasks(self, anns, alpha=1, draw_border=False):
    #     """
    #     Generate and return colored masks for specified annotations with optional transparency,
    #     including a white 2px border around each object mask.
    #     :param anns: Annotations to display, each with segmentation info.
    #     :param alpha: Opacity level for the masks.
    #     :return: A numpy array of masks with applied colors, alpha transparency, and white borders.
    #     """
    #     if len(anns) == 0:
    #         return np.zeros((0, 0, 4))
    ...
    #             border = binary_dilation(edge, iterations=2) - edge
    #             img[border == 1] = [1, 1, 1]

    #         combined_mask = np.dstack((img, mask * alpha))
    ...
    def showAnns(self, anns, draw_bbox=False, ax=None):
        """
        Display the specified annotations.
        :param anns (array of object): annotations to display
        :return: None
        ...
        if datasetType == 'instances':
            ...
            for ann in anns:
                ...
                if 'segmentation' in ann:
                    ...
                    # show score
                    # x, y, w, h = ann['bbox']
                    # score = f"{float(ann['score']):.2f}" if 'score' in ann else ''
                    # ax.text(x + w//2, y + h//2, score, fontsize=12, color='black', bbox=dict(facecolor='white', alpha=0.5, edgecolor='white'))
