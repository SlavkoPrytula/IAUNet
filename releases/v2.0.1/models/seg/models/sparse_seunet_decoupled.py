...
@MODELS.register(name="sparse_seunet_decoupled")
class SparseSEUnet(BaseModel):
    """
    Base SparseUnet with decoupled multi-level mask and instance braches
    """
    def __init__(
        self,
        cfg: cfg,
        ...
        for _ in range(self.n_levels):
            if len(self.up_conv_layers_mb) == 0:
                ...
                upconv = DoubleConv((self.n_filters // 4) * 5 + 2 * self.n_filters, self.n_filters)
    ...
    def forward(self, x, idx=None):
        ...
        def go_up(x):
            ...
            for i in range(self.n_levels):
                ...
                mb = self.up_conv_layers_mb[i](mb)
                mb = self.up_se_blocks_mb[i](mb)
