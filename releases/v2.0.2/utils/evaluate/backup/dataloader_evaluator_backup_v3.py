...
class DataloaderEvaluator(Evaluator):
    ...
    def forward(self, model, dataloader):
        ...
        for step, batch in enumerate(dataloader):
            ...
            # probabilities = pred['pred_logits'][0, ...].softmax(-1)
            # # probabilities = F.softmax(pred['pred_logits'][0, ...], dim=1)
            ...
            probs = pred['pred_logits'].sigmoid()
            # probs = pred['pred_logits'][0, ...].softmax(-1)
