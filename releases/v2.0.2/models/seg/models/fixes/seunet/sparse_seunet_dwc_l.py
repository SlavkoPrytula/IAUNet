...
@MODELS.register(name="sparse_seunet_dwc_l")
class SparseSEUnet(BaseModel):
    def __init__(
        ...
        for i in range(self.n_levels):
            ...
            if i != self.n_levels - 1:
                upconv = nn.Sequential(
                    # Block(self.embed_dims[-(i+1)]*2),    # ([C + skip], H, W) -> (D, H, W)
                    # Block(self.embed_dims[-(i+1)]*2),    # ([C + skip], H, W) -> (D, H, W)
                    # nn.Conv2d(self.embed_dims[-(i+1)]*2, self.embed_dims[-(i+1)-1], kernel_size=1),
                    # nn.BatchNorm2d(self.embed_dims[-(i+1)-1]),
                    # nn.ReLU(),
                    # Block(208),
                    # DWCFusion(self.embed_dims[-(i+1)]*2, self.embed_dims[-(i+1)-1])
                ...
                upconv = nn.Sequential(
                    # Block(self.embed_dims[-(i+1)]*2),    # ([C + skip], H, W) -> (D, H, W)
                    # Block(self.embed_dims[-(i+1)]*2),    # ([C + skip], H, W) -> (D, H, W)
                    # nn.Conv2d(self.embed_dims[-(i+1)]*2, self.embed_dims[-(i+1)]*2, kernel_size=1),
                    # nn.BatchNorm2d(self.embed_dims[-(i+1)-1]),
                    # nn.ReLU(),
                    # Block(208),
                    # DWCFusion(self.embed_dims[-(i+1)]*2, self.embed_dims[-(i+1)]*2)    # ([C + skip], H, W) -> ([C + skip], H, W)
                    FusionConv(self.embed_dims[-(i+1)]*2, self.embed_dims[-(i+1)]*2)
                    )
            self.up_skip_fusion_layers.append(upconv)

        # mask branch.
        self.mask_branch = nn.ModuleList([])
        self.mask_branch.append(MaskBranch(in_channels=self.embed_dims[0]*2+2, out_channels=256, num_convs=4))
        # for i in range(self.n_levels):
        #     if i == 0:
        #         self.mask_branch.append(MaskBranch(in_channels=self.embed_dims[-(i+1)], out_channels=256, num_convs=self.num_convs))
        #     elif i != self.n_levels - 1:
        #         self.mask_branch.append(MaskBranch(in_channels=self.embed_dims[-(i+1)]+256, out_channels=256, num_convs=self.num_convs))
        #     else:
        #         self.mask_branch.append(MaskBranch(in_channels=self.embed_dims[-(i+1)] + self.embed_dims[-1], num_convs=self.num_convs))
        ...
        # for i in range(self.n_levels):
        #     if i == 0:
        #         self.prior_instance_branch.append(PriorInstanceBranch(self.embed_dims[-(i+1)], out_channels=256, num_convs=self.num_convs))
        #     elif i != self.n_levels - 1:
        #         self.prior_instance_branch.append(PriorInstanceBranch(self.embed_dims[-(i+1)]+256, out_channels=256, num_convs=self.num_convs))
        #     else:
        #         self.prior_instance_branch.append(PriorInstanceBranch(in_channels=self.embed_dims[-(i+1)] + self.embed_dims[-1], out_channels=256, num_convs=self.num_convs))
