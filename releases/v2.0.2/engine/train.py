...
def train_one_epoch(
    ...
    if epoch % 10 == 0:
        ...
        maskness_scores = output['pred_scores'].sigmoid()
        maskness_scores = maskness_scores[0, :, 0].cpu().detach().numpy()
        print(scores.shape, maskness_scores.shape)
        maskness_scores = np.round(maskness_scores, 2)
        print("pred ious:")
        ...
        titles = [
            f"conf: {score:.2f}, iou: {maskness_score:.2f}"
            for score, maskness_score in zip(scores, maskness_scores)
        ]
        ...
        if "aux_outputs" in output:
            for i, aux_outputs in enumerate(output['aux_outputs']):
                ...
                titles = [', '.join([f"({class_idx}, {score:.2f})" for class_idx, score in zip(range(scores.shape[1]), score)]) for score in scores]
