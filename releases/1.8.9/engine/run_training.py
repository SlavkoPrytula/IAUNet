...
def run_training(cfg, model, losses, optimizer, train_loader, valid_loader, 
    ...
    for epoch in range(0, num_epochs):
        ...
        if epoch % 10 == 0:
            ...
            msg = ('%25s,' * len(metrics) % tuple(metrics)).replace(',', '') + '\n'
            msg += ('%25.5g,' * len(vals) % tuple(vals)).replace(',', '')
            
            s = '' if cfg.csv.exists() else (('%25s,' * (len(metrics) + 1) % tuple(['epoch'] + metrics)).rstrip(',') + '\n')  # header
            with open(cfg.csv, 'a') as f:
                f.write(s + ('%25.5g,' * (len(metrics) + 1) % tuple([epoch] + vals)).rstrip(',') + '\n')
