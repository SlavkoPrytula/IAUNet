...
class Brightfield_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            data = self.transform(image=image, mask=mask, prob_map=prob_map)
            image = data['image']
            mask = data['mask']
    ...
    def get_prob_map(self, img_id: int):
        ...
        prob_map[prob_map!=0] = normalize2range(prob_map[prob_map!=0], (0.1, 0.9))
...
if __name__ == "__main__":
    ...
    time_s = time.time()
    dataset = Brightfield_Dataset(df, "train", cfg.train.size, 
