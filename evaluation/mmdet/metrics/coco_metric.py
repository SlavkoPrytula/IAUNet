...
from evaluation.mmdet.metrics.base_metric import BaseMetric
from evaluation.mmdet.api_wrappers import COCO, COCOeval, COCOevalMP
from evaluation.mmdet.functional import eval_recalls
...
@METRICS.register(name="CocoMetric")
class CocoMetric(BaseMetric):
    ...
    def results2json(self, results: Sequence[dict],
        ...
        for idx, result in enumerate(results):
            ...
            # print(mask_scores)
            # mask_scores = np.ones(mask_scores.shape)
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            if self._coco_api is None:
                ...
                # ADDED MANUALLY
                # TODO: this is not working properly for subset evaluation
                if 'masks' in data_sample['instances']:
                    gt['anns']['masks'] = encode_mask_results(
                        data_sample['instances']['masks'].detach().cpu().numpy()) if isinstance(
                            data_sample['instances']['masks'], torch.Tensor) else data_sample['instances']['masks']
                    
                gt['anns'] = [{'mask': m, 
                               'bbox': self.xyxy2xywh(b.cpu().numpy()), 
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for metric in self.metrics:
            ...
            if self.coco_api == 'COCOevalMP':
                coco_eval = COCOevalMP(self._coco_api, coco_dt, iou_type)
            elif self.coco_api == 'COCOeval':
                coco_eval = COCOeval(self._coco_api, coco_dt, iou_type)
            # elif self.coco_api == 'COCOeval_nofp':
            #     coco_eval = COCOeval_nofp(self._coco_api, coco_dt, iou_type)
...
#         print(f'self.img_ids: {len(self.img_ids)}')
#         print(f'self.cat_ids: {len(self.cat_ids)}')
...
#                 raise KeyError(f'{metric} is not in results')
#             try:
#                 print(f'loading result_files[{metric}]')
#                 predictions = load(result_files[metric])
#                 if iou_type == 'segm':
#                     # Refer to https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocotools/coco.py#L331  # noqa
#                     # When evaluating mask AP, if the results contain bbox,
...
#                     for x in predictions:
#                         x.pop('bbox')
#                     print('bbox' in predictions[0])
                    
#                 coco_dt = self._coco_api.loadRes(predictions)

#                 for img_id in coco_dt.getImgIds():
#                     ann_pred_custom = self._coco_api.loadAnns(self._coco_api.getAnnIds(imgIds=img_id))
#                     ann_pred_naive = coco_dt.loadAnns(coco_dt.getAnnIds(imgIds=img_id))
#                     if ann_pred_custom != ann_pred_naive:
#                         print(f"Mismatch for image {img_id}")


#                 gt_image_ids = set(self._coco_api.getImgIds())
#                 pred_image_ids = set(coco_dt.getImgIds())
...
#             #     coco_eval = COCOevalMP(self._coco_api, coco_dt, iou_type)
#             # else:
#             coco_eval = COCOeval(self._coco_api, self.coco_dt, iou_type)
...
#                     eval_results[item] = val
#             else:
#                 print('doing evaluate -> accumulate -> summarize')
#                 coco_eval.evaluate()
#                 coco_eval.accumulate()
#                 coco_eval.summarize()
#                 if self.classwise:  # Compute per-category AP
